...
fun ReportIncidentScreen(token: String, onBack: () -> Unit) {
    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(20.dp)) {
        ...
        OutlinedButton(onClick = { captureLocation() }, enabled = !locating, modifier = Modifier.fillMaxWidth()) {
            Text(if (locating) "GETTING LOCATION..." else "📍 USE MY CURRENT LOCATION")
        }
        if (latitude != null && longitude != null) Text("GPS: %.6f, %.6f".format(latitude, longitude), style = MaterialTheme.typography.bodySmall)
        Spacer(Modifier.height(16.dp))
        ...
        Button(onClick = {
            ...
            scope.launch {
                val result = withContext(Dispatchers.IO) {
                    client.createReport(EmergencyReportRequest(type, location.trim(), description.trim().ifBlank { null }, latitude, longitude), token)
                }
                submitting = false
                message = when (result) {
                    ReportResult.SUCCESS -> "Report submitted — status: PENDING"
                    ReportResult.AUTH_REQUIRED -> "Session expired. Please log in again."
                    ReportResult.FAILURE -> "Could not submit report. It will need another attempt when connected."
                }
            }
