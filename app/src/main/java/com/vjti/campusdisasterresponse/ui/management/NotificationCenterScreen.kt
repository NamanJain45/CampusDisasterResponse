...
fun NotificationCenterScreen(token: String, onBack: () -> Unit) {
    ...
    fun load() {
        scope.launch {
            ...
            result.onSuccess {
                notifications = it
                message = if (it.isEmpty()) "No notifications" else "${it.count { n -> n.readAt == null }} unread"
    ...
    Column(Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
            ...
            TextButton(onClick = { scope.launch { client.markAllRead(token); load() } }) { Text("READ ALL") }
        }
        Text(message)
                ...
                Card(Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(5.dp)) {
                        ...
                        if (notification.readAt == null) {
                            TextButton(onClick = { scope.launch { client.markRead(token, notification.id); load() } }) { Text("MARK READ") }
                        }
                    }
                }
            }
