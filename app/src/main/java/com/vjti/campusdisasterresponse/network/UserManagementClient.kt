...
class UserManagementClient(
    ...
    suspend fun listUsers(token: String): Result<List<ManagedUser>> =
        request(token, "GET") as Result<List<ManagedUser>>

    suspend fun createUser(token: String, name: String, email: String, password: String, role: String): Result<ManagedUser> =
        request(
            token,
            "POST",
    ...
    suspend fun deleteUser(token: String, userId: String): Result<Unit> = withContext(Dispatchers.IO) {
        runCatching {
            val connection = URL("$baseUrl/api/v1/users/$userId").openConnection() as HttpURLConnection
            try {
                ...
                if (connection.responseCode !in 200..299) {
                    throw IllegalStateException("Unable to remove user (HTTP ${connection.responseCode})")
                }
            } finally {
    ...
    private suspend fun request(token: String, method: String, body: JSONObject? = null): Result<Any> =
        withContext(Dispatchers.IO) {
            runCatching {
                val connection = URL("$baseUrl/api/v1/users/").openConnection() as HttpURLConnection
                try {
