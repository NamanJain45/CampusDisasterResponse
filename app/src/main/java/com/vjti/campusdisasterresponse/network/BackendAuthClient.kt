...
sealed class AuthLoginResult {
    data class Success(val token: String) : AuthLoginResult()
    data class Failure(val message: String) : AuthLoginResult()
}
                ...
                try {
                    ...
                    if (responseCode !in 200..299) {
                        return@withContext AuthLoginResult.Failure(
                            responseMessage(responseBody, "Login failed (HTTP $responseCode)")
                        )
                    }
                    val token = runCatching { JSONObject(responseBody).optString("token") }.getOrDefault("")
                    if (token.isBlank()) AuthLoginResult.Failure("Backend returned no token")
                    else AuthLoginResult.Success(token)
                } finally {
                    connection.disconnect()
                }
            } catch (error: Exception) {
                AuthLoginResult.Failure(error.message ?: "Unable to reach backend")
            }
        }

    private fun responseMessage(body: String, fallback: String): String =
