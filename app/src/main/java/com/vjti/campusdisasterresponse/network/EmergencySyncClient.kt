    ...
    fun syncEvent(event: EmergencyEvent, token: String): SyncTransmissionResult {
        return try {
            ...
            try {
                ...
                val processedActions = JSONObject(responseBody).optJSONArray("processedActions")
                    ?: return SyncTransmissionResult.FAILURE
                for (index in 0 until processedActions.length()) {
                    if (processedActions.optString(index) == event.id) return SyncTransmissionResult.SUCCESS
                }
