...
class EmergencyAlertClient(private val baseUrl: String = "http://192.168.0.115:3000") {
    fun getActiveAlerts(token: String): List<ServerEmergencyAlert> {
        ...
        try {
            ...
            return buildList { for (i in 0 until array.length()) { val j = array.getJSONObject(i); add(ServerEmergencyAlert(j.optString("id"),j.optString("title"),j.optString("message"),j.optString("severity"),j.optLong("createdAt"))) } }
        } finally { connection.disconnect() }
    }
}

data class ServerEmergencyAlert(val id:String,val title:String,val message:String,val severity:String,val createdAt:Long)
